...
function Get-FlutterBundledDartVersion {
    ...
    $match = [regex]::Match(
        $Text,
        '(?im)\bDart\s+(?<version>\d+(?:\.\d+){1,3}(?:[-+][0-9A-Za-z.-]+)?)'
    )
...
foreach ($candidate in $flutterRoots) {
    ...
    Add-UniqueInstallation -List $flutterInstallations -Path ([string]$candidate.path) -Version $(if ($active) { $flutterVersion } else { $null }) -Active $active -Source ([string]$candidate.source)
}
...
foreach ($candidate in $bundledDartCandidates) {
    Add-UniqueInstallation -List $dartInstallations -Path ([string]$candidate.path) -Version $candidate.version -Active $false -Source filesystem
}
...
Add-UniqueVersion -List $dartVersions -Version $bundledDartVersion
...
$adbVersion = if ($adbResult.Found) { Get-AdbVersionRecord -Text $adbResult.Captured } else { $null }
...
$adbComponent = New-CommandComponent -ComponentId 'adb' -Name 'Android Debug Bridge' -Command 'adb' -Arguments @('version') -EvidenceId 'mobile.adb.version' -VersionParser {
    param($text)
    Get-AdbVersionRecord -Text $text
